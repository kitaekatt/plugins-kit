...
### Correction to P3 and P11: the launch-banner separator (2026-08-20)

**The banner separator is U+00B7 MIDDLE DOT, not an asterisk.** Captured live
from Claude Code 2.1.238 during the B2 gate: `cat -A` renders the line as
`backgrounded M-BM-7 ff97012c$`, i.e. the bytes `C2 B7`. P3 and P11 above both
record the separator as `*`, observed on 2.1.233.

**Which of the two explanations holds is UNRESOLVED** and should not be guessed
at: either the CLI changed its separator between 2.1.233 and 2.1.238, or the
original probes transcribed the middle dot as an asterisk -- plausible, because
the driver's own subprocess call decoded UTF-8 as cp1252, so anything reading
that output saw a mojibaked two-character sequence rather than a middle dot.

**What this cost.** `_BG_LAUNCH_BANNER_RE` was written from the recorded `*`, so
in shipped 0.11.0 it could never match. Every dispatch aborted the wave and
orphaned the live session it had just spawned, because the corpse cleanup is
gated on the same unparsed id. Fixed in 0.12.0.

**The durable rule, which is what matters more than the character:** the driver
must not depend on the separator at all. A cosmetic change to a human-facing
banner is not a contract, and treating one as a contract is what turned a
formatting detail into a total dispatch failure. Match the id, tolerate anything
between it and the word `backgrounded`, and pin the parse with a fixture
captured FROM the CLI rather than one written by hand -- the previous tests
invented the banner string themselves and so agreed with the bug.
