...
def test_rendered_code_review_skills_may_carry_a_generated_banner():
    """The ten deliberately-committed generator outputs are exempt.

    They carry an `@generated` banner so a code review classifies them as
    machine-emitted and looks at the generator instead of auditing output nobody
    can hand-edit. This rule matches on exactly that banner, so without the
    exemption the banner could not be committed at all.
    """
    banner = b"<!-- @" + b"generated by scripts/gen_code_review_skills.py -->\n"
    for path in sorted(guard.GENERATED_ARTIFACT_EXEMPT_PATHS):
        assert guard.inspect_file(path, len(banner), banner, set(), []) == []


def test_the_exemption_covers_only_the_generated_artifact_rule():
    """Every other rule still applies to an exempt path.

    The exemption answers "is a banner allowed here", not "is this path
    unchecked" -- an exempt file that is oversized, or that leaks a local term,
    is still refused.
    """
    path = sorted(guard.GENERATED_ARTIFACT_EXEMPT_PATHS)[0]
    banner = b"<!-- @" + b"generated -->\nNEEDLE\n"
    assert rules(guard.inspect_file(path, len(banner), banner, set(), ["needle"])) == {
        "local term"
    }
    oversized = guard.MAX_STAGED_FILE_BYTES + 1
    assert rules(guard.inspect_file(path, oversized, None, set(), [])) == {"size"}


def test_the_exemption_does_not_widen_to_neighbouring_paths():
    """A fixed path list, not a pattern.

    A generated file added later under the same skill must still be refused
    until someone decides it belongs on the list -- that decision is the rule.
    """
    banner = b"<!-- @" + b"generated -->\n"
    neighbour = "plugins/git-kit/skills/git-code-review/references/new-doc.md"
    assert neighbour not in guard.GENERATED_ARTIFACT_EXEMPT_PATHS
    assert rules(guard.inspect_file(neighbour, len(banner), banner, set(), [])) == {
        "generated artifact"
    }
...
def test_exempt_paths_match_the_generator_targets():
    ...
    rendered = {
        Path(path).relative_to(ROOT).as_posix() for path in gen.targets()
    }
    assert rendered == set(guard.GENERATED_ARTIFACT_EXEMPT_PATHS)
