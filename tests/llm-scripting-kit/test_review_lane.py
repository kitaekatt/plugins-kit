"""Tests for llm_scripting_kit.review_lane.

The completion seam is faked rather than imported from a plugin venv. These
tests exercise the shared endpoint-lane policy, retry behavior, budget guard,
and JSON envelope without making a network request.
"""
...
from llm_scripting_kit import review_lane as lr


_MODULE_PATH = (
    Path(__file__).resolve().parents[2]
    / "plugins" / "llm-scripting-kit" / "lib" / "llm_scripting_kit" / "review_lane.py"
)
...
def seam(monkeypatch: pytest.MonkeyPatch):
    """Install fake seam and model-registry modules for isolated unit tests."""
    ...
    monkeypatch.setattr(lr, "BackendOptions", BackendOptions)
    monkeypatch.setattr(lr, "HaltError", FakeHaltError)
    monkeypatch.setattr(lr, "create_backend", create_backend)
    monkeypatch.setattr(lr, "EndpointResolveError", FakeEndpointResolveError)
    monkeypatch.setattr(lr, "discover_model_entries", lambda **_kwargs: control.entries)
    # A transport lane pre-flights the openai SDK, which is an OPTIONAL
    # dependency of the kit and is not installed for this suite. The default
    # is present so transport dispatch can reach the fake backend.
