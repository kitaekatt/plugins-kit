...
# tracked file is exactly what the repo's pre-commit guard refuses.
BANNER = "# " + "Generated by" + " some-codegen 1.2 -- edits will be lost"


def _added_section(ident: str, lines: list[str]) -> dict:
    """A diff section whose added content is exactly `lines`."""
    body = "".join(f"+{line}\n" for line in lines)
...
class TestAssembleBundleGenerated:
    def test_generated_file_excluded_from_chunks_and_surfaced(self, tmp_path):
        core = assemble_bundle(
            preamble="",
            sections=[
                _added_section("gen/stub.py", [BANNER, "def a(): ..."]),
                _added_section("src/b.py", ["def b(): ..."]),
            ],
            files=[
                {"identifier": "gen/stub.py", "local": None},
                {"identifier": "src/b.py", "local": None},
            ],
            bundle_dir=tmp_path / "b",
            max_chunk_bytes=1024 * 1024,
        ...
        assert "gen/stub.py" not in all_text
        assert "src/b.py" in all_text

    def test_large_hand_written_file_is_still_chunked(self, tmp_path):
        """Size alone is evidence of nothing -- only content decides."""
        big = ["def f_%d(): return %d" % (n, n) for n in range(4000)]
        core = assemble_bundle(
            preamble="",
            sections=[_added_section("src/big.py", big)],
            files=[{"identifier": "src/big.py", "local": None}],
            bundle_dir=tmp_path / "b",
            max_chunk_bytes=1024 * 1024,
            workspace_root=None,
        )
    ...
    def test_modified_generated_file_detected_from_disk(self, tmp_path):
        ...
        core = assemble_bundle(
            preamble="",
            sections=[_added_section("gen/stub.py", ["def zzz(): ..."])],
            files=[{"identifier": "gen/stub.py", "local": str(local)}],
            bundle_dir=tmp_path / "b",
            max_chunk_bytes=1024 * 1024,
            workspace_root=None,
        )
    ...
    def test_generated_file_still_contributes_submit_gates(self, tmp_path):
        ...
        (ws / "CLAUDE.md").write_text(
            "\n".join(
                [
                    "# Root",
                    "",
                    "**Submit gate:** Regenerate the stubs.",
                    "Applies to:",
                    "- gen/",
                    "",
                ]
        ...
        local = ws / "gen" / "stub.py"
        local.write_text(BANNER + "\ndef a(): ...\n", encoding="utf-8")
        core = assemble_bundle(
            preamble="",
            sections=[_added_section("gen/stub.py", ["def a(): ..."])],
            files=[{"identifier": "gen/stub.py", "local": str(local)}],
            bundle_dir=tmp_path / "b",
            max_chunk_bytes=1024 * 1024,
            workspace_root=ws,
