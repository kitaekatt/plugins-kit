  ...
  facts:
    ...
    - id: remediation_phases
      ...
      detail: |
        ...
        Phase 2 (fix-all): aggregates remaining failures into a single fix-all message. additionalContext (seen by the agent) carries numbered remediation steps; systemMessage (seen by the user) carries the bootstrap log of what was checked and what failed -- except once failures have been collapsed onto a root cause (the elevation aggregate), where it carries one line per root cause plus a pointer to bootstrap.log instead (see references/remediation-reference.md). User types "fix-all" to run the auto-remediation; the failing checks re-run every session until clean, so no separate confirmation is needed after manual fixes.
      example:
