...
launching shell. That bash is never WSL's `System32\bash.exe`:
`tool_check.resolve_bash()` skips it even when PATH lists System32 first, and falls
back to Git for Windows' bash. WSL's launcher would run the command inside a Linux
distro, with the disk at `/mnt/c`.
...
| `alwaysUpdate` | No | Refresh the marketplace **clone/listing** against its remote on every pass. Every refresh runs before any plugin version check of the pass, so a plugin with a `plugins[]` entry (declared, or self-registered by bootstrap for a bootstrap-dependent plugin) is updated to a newly published version in the **same** pass. A plugin with no `plugins[]` entry anywhere moves only through Claude Code's `autoUpdate: true` (set via an `extraKnownMarketplaces` block in a settings.json); for it, `alwaysUpdate` keeps only the listing fresh (update mechanics: references/plugin-reload-lifecycle.md) -- **Ignored while `pin` is set** (a one-line warning action is emitted) |
