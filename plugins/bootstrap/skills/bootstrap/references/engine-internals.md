...
1. **SessionStart hook** (instant): Runs only the skip gates on the foreground path, emits `{"continue": true, "suppressOutput": true}`, then dispatches ALL provisioning work — PATH setup, Python ensure/download, Windows registry PATH writes, the engine launch (`--background`) — as a detached `_provision` subshell with stdin and stdout redirected to `/dev/null` and stderr to a file. The redirection is load-bearing, not hygiene: Claude Code blocks session readiness on the hook's process exit AND stdout-pipe EOF (measured 2026-07-20; a child that inherits stdout blocks exactly like foreground work). Foreground cost is ~tens of ms on every path, including cold start; contract pinned by `tests/bootstrap/test_sessionstart_detach.py`.

   **stderr goes to `provision_stderr.log`, not `/dev/null` (0.86.1).** What holds the session is a child inheriting the parent's PIPE, not a child holding an fd on disk, so a file satisfies the measured constraint exactly as `/dev/null` did. It is truncated on every launch, so it is empty after a healthy pass and holds exactly one failing run otherwise. `_provision` also installs its own `EXIT` trap (`_provision_crash`) that, on a non-zero exit, flushes the shell log and writes `bootstrap_display.pending` naming the exit code, the wrapper path, the captured stderr, and the `--console` reproduction. Both exist because a fatal SHELL error inside `_provision` previously left NO trace anywhere: the log helper holds its entries in memory until the end, the engine that owns `engine_output.log` has not started yet, and nothing wrote the display file — so the failure was indistinguishable from a clean pass, which is also silent. That is how a `set -u` abort went unnoticed on macOS -- a possibly-empty `"${ARR[@]}"` is fatal under `set -u` on the bash 3.2 that macOS ships.
...
3. **UserPromptSubmit hook** (`hooks/userpromptsubmit/bootstrap-display.sh`, every prompt, ~0ms when idle): Checks for `bootstrap_display.pending`. If present, emits its contents **stamped with the message's own age** and renames it to `bootstrap_display.displayed`. If absent, exits immediately with no output. The `.displayed` file is preserved for debugging and as a handshake signal -- the engine can overwrite it with a new `.pending` file when fresh results are available. **Why UserPromptSubmit (not Stop)**: UserPromptSubmit supports `hookSpecificOutput.additionalContext`, which injects the log + remediation directives into Claude's context; Stop hooks reject `hookSpecificOutput` via schema validation. The pending JSON therefore carries `systemMessage` (user-facing) plus `hookSpecificOutput` with `hookEventName: "UserPromptSubmit"` and `additionalContext` (Claude-facing). There is no `decision`/`reason` field.

**The age stamp (`bootstrap_lib/display_relay.py`).** The pass that WRITES a pending file and the prompt that READS it are not the same moment: the file waits on disk until *some* session's first prompt consumes it, so a session that starts after a skipped pass (cooldown or session guard) can be handed a verdict produced long before it -- and nothing in the payload recorded when the pass ran, so "Setup issues found" read as a statement about the machine *now*. The display hook therefore relays through `display_relay.py`, which derives the age from the pending file's mtime (an atomic rename sets it to the instant the verdict was written -- no new state file) and folds it into **both** message surfaces: `systemMessage` gets `[bootstrap pass finished 30m ago -- this may not reflect the machine's current state]`, and `additionalContext` gets a leading `TIMING:` line telling Claude not to present it as the current state. The stamp is **unconditional** -- a fresh message reads `just now`, because a marker that appeared only when stale would make its own absence carry meaning. The age rides inside the existing strings rather than in a new top-level key, since the payload is schema-validated hook JSON. On success the relay consumes the file itself and the hook must not also `cat` it; on any failure (unparseable payload, or no Python at all on a fresh machine -- where the pending file is often the message saying exactly that) it exits 1, prints nothing, and the hook's plain `cat` + `mv` runs unchanged.
...
`bootstrap_lib/engine.py::_run_codex_hook_setup`. It resolves the Git root when
there is one, checks the applicable Git and Perforce ignore policies, and only
then writes the machine-local project file `<project>/.codex/hooks.json`
through `bootstrap_lib/codex_hook.py`. If policy is missing, bootstrap reports
remediation and defers the write; a failed pass or console diagnostic also does
not create the adapter. The write path is project-ephemeral, not derived from
the plugin script location, and the JSON preserves unrelated Codex hooks while
replacing only bootstrap-owned entries. Existing hook-file modes are preserved,
symlink targets are refused, and concurrent merges are serialized.

The generated `SessionStart` matcher covers `startup` and `resume`. Its stable
command is the user lever `bootstrap codex-hook`, rather than a versioned cache
path. The CLI runs the full engine synchronously without the Claude background
relay, so the engine's stdout is Codex `hookSpecificOutput` with
...
**Malformed config files.** A `config.yaml` the engine is about to read -- the data-dir file in the `config` phase or the per-project file in the `project_config` phase -- that fails to parse, or whose top level is not a mapping, is reported with ONE action entry naming the file and the parse error, and every write to that file is skipped for the rest of the pass. The engine never replaces an unreadable config with defaults or autodetected values; the user's bytes stay on disk until they fix the file. (`config_check.load_yaml_config` raises `config_resolve.ConfigError`; an absent file still reads as empty.)
