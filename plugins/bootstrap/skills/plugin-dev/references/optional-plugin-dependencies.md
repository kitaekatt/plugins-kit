...
|---|---|---|---|
| git-kit / p4-kit `run_review_lane.py` wrapper (endpoint lane) | REFUSE | Exit 2 with a reason; SKILL.md forbids substitution; `## Lane failures` carries the gap into the review; `openai` is pre-flighted by name in `llm_scripting_kit.review_lane`. The identical wrapper distinguishes an absent owner from a present owner whose frontier module or `main` entry point is missing, so the two states name different `claude plugin install` / `update` commands; the owner version for the frontier is 0.29.0. | The `endpoint-dispatch` extra is not enableable by a consumer, so a transport endpoint is refused rather than provisioned. |
