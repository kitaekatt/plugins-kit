...
capability_skill:
  ...
  capabilities:
    ...
    - id: list
      ...
      gotchas:
        - "Contract: one parseable line per task -- 'id  status  priority  title' (absent fields '-'); dedupes by canonical path; classifies each via validate. Remote tasks list as '<path> @<host>  remote  -  -' (not locally resolvable). Folderless non-tmp refs read as archived; folderless local tmp refs as orphaned. ARCHIVED tasks are omitted by default -- the listing is the working set -- and appear only with --status archived. Exit 0 even when empty; notes go to stderr."
