...
capability_skill:
  ...
  capabilities:
    ...
    - id: validate
      ...
      gotchas:
        ...
        - "Advisory note: lines (NOT findings; exit code unaffected) also print to stderr: approaching-budget (doc past its healthy target -- CLAUDE.md 250, plan.md 300), dominant-section (one ## section over half of CLAUDE.md/plan.md), session-diary (dated narrative accreting in CLAUDE.md), version-control-unverified (a dev/tasks folder outside any git repo -- the scripts cannot check other VCS; ensure it is submitted, e.g. via p4), version-control-ignored (a dev/tasks folder git is configured to ignore and carries no tracked file inside -- git is present but will never hold it, so nothing is recoverable once removed; relocate anything that must outlive the task and declare it with update --durable-output). Act on the size notes at the next rotation; thresholds and remedies in references/handoff-template.md 'Document size budgets'."
    - id: work
    ...
    - id: move
      ...
      operation: >-
        ~/.claude/plugins/data/plugins-kit/awesome-kit/.venv/bin/python
        "${CLAUDE_PLUGIN_ROOT}/skills/task/scripts/task.py"
        move <ref> <tmp|dev/tasks> [--root PATH]
      gotchas:
  ...
  gotchas:
    ...
    - "orchestrate is not a prose requirement you have to carry -- `work` EMITS it as the first Skill(...) line of the initialization block, ahead of the task's own skills. Invoke it from that line like any other. It owns the delegation economics (what goes to a background agent vs inline) and the model-tier selection for every agent dispatch; this skill does not restate them. Name `model` explicitly per its tiers -- a model-less dispatch skips the routing."
