...
capability_skill:
  ...
  gotchas:
    ...
    - "orchestrate owns the delegation economics (background agent vs inline) and the model tier for every agent dispatch; this skill does not restate them. `work` emits it as the first Skill(...) line -- invoke it from there. Name `model` explicitly per its tiers; a model-less dispatch skips the routing."
