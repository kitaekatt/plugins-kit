...
- `change_id` (`bundle.change_id`) = the CL number. It is the outer ledger
  key -- entries are bucketed per change.
- `baseline` (`bundle.ledger_baseline`) = a hash over the CL's shelf fingerprint (content) plus its per-file (rev, action) map (identity). Every recorded entry
...
per-change bundle dirs: `~/.claude/plugins/data/plugins-kit/p4-kit/reviews/ledger.json`. Never written into the user's repo
working tree. Shape:
