...
- **move_into == ``p4 reopen -c <cl> <path>`` per exact path -- NEVER a
  wildcard.** A wildcard reopen (``p4 reopen -c <cl> <path>/...``) moves every
  opened file under the path, including files organized into other pending CLs,
  silently destroying CL organization that is recorded nowhere. A path
  containing ``...`` or ``*`` is rejected outright.
...
class P4Vcs:
    ...
    def move_into(self, changeset: P4Changeset, paths: list) -> None:
        """Reopen each of ``paths`` into ``changeset``'s CL and record them.
        ...
        for path in paths:
            ...
            self._p4("reopen", "-c", changeset.cl, p)
