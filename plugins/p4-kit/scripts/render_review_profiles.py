#!/usr/bin/env python3
"""Render the resolved code-review profile configuration for p4-kit.

Thin plugin-venv entry point: re-execs under p4-kit's bootstrap-provisioned
venv, then delegates to the shared resolver in
bootstrap_lib.code_review.review_profiles, which owns resolution, merging,
and rendering. This script exists only so the p4-code-review skill can
