...
The plugin ships one CLI script at `${CLAUDE_PLUGIN_ROOT}/scripts/llm_scripting_kit_cli.py`. This skill covers the three key-management subcommands; see README.md for the CLI's full verb set (endpoints, probe, usage, describe, seats, models, resolve, complete, request-schema).
