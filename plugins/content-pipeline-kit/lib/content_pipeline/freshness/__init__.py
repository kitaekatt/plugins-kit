...
This subsystem is pure, with no LLM, VCS, or I/O side effects to mock, which
makes it independently reusable. Submodules:
...
1. **Uniform string canonicalization (no legacy raw-UTF-8 path).** A legacy
   representation hashed a bare string as raw UTF-8 bytes while hashing the
   newer dict shape via sorted-key JSON, preserving existing hashes through a
   field-shape migration. The generic library has no legacy corpus to stay
   byte-compatible with, so ``hashing.canonical_bytes`` routes every
   ...
   the pre-migration encoding is dropped.

2. **Empty recorded hash on a present machine value classifies as STALE, not
   MISSING.** Earlier classifiers assigned different bucket labels to a
   present machine value with an empty ``inputs_hash``. The generic
   ``classify`` follows this package's own state definitions -- ``MISSING``
...
3. **``EXCLUDED`` is checked after ``HUMAN``.** Earlier classifiers each
   covered only one of these tiers. The combined classifier orders
