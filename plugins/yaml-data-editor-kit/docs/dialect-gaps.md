...
| D-7 | **RESOLVED** (dialect.md, "including a recursive `**` segment spanning") -- two sources claiming different regions of ONE file; `single`'s wording reads as a conflict | `manifest.yaml`, claimed by a `single` source and a `rows` source with `key:` for the same file (see a consuming profile's own `FINDING D-7` comment for a worked instance) | `single` covers keys claimed by no sibling; a same-path `rows` source with `key:` owns and validates only that key, and excluded-key comment addresses name that owner and a working replacement even when the `single` type does not declare the key. Whole-document siblings (keyless `rows`, `keyed_map`, a `file_per_record` glob that actually matches the concrete file -- including recursive `**` across zero or more directories -- or another `single`) are refused as load errors; duplicate keyed `rows` owners are refused only when a `single` source also occupies that path. Closed in yaml-data-editor-kit 0.11.0; two keyed `rows` sources with no `single` remain outside this ruling. |
