...
def bootstrap(ctx: Any) -> None:
    ...
    ctx.add_failure(
        ...
        agent_msg=(
            ...
            "  3. On \"I'm ready\": start `hue-kit pair --no-wait` IN THE "
            "BACKGROUND, then IMMEDIATELY tell the user \"press the round "
            "button on top of the bridge now\". The command blocks for up to "
            "30s polling, so the instruction must not wait on it -- otherwise "
            "the user is told to press only after the window has burned.\n"
