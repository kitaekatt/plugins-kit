    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          ...
          taxonomy: {
            type: 'string',
            enum: ['A_misclassified_skill_ref', 'B_graduate_to_skill', 'C_fold_into_claude_md', 'D_move_into_existing_skill', 'E_crp_split', 'F_chained_reference', 'G_claude_md_back_reference', 'H_orphan', 'I_duplicates_skill', 'J_size_signal', 'K_unclassified', 'L_readme_stranded_fact', 'M_generated_missing_provenance', 'N_broken_link_identified_target', 'O_non_ascii_lookalike', 'P_foreign_absolute_path', 'Q_line_drift', 'R_stale_anchor', 'S_ancestor_convention_violation', 'T_dated_record_missing_provenance', 'N_user_standard_violation', 'none'],
...
2. Read the project-doc standards at ${refs.criteria}. This file is self-contained: every testable rule is stated with the cohesion principle it derives from. Do NOT load any other framework document. (Recap: run the PD-12 record-role check before the criteria it can exempt, and apply PD-2's exclusions before the nursery test. For each remaining candidate, stabilized content moves to its TRIGGER-APPROPRIATE mature home -- a skill is NOT the default home for all reference content. The criteria check whether the content has matured past a standalone doc, serves one reading task, is reachable, and does not duplicate a skill.)
2a. DATED RECORD ROLE (PD-12, criterion dated_record_role): inspect the opening (first ~20 lines) BEFORE PD-2, PD-8, PD-H1, N, or R. Assign the record role ONLY when the opening declares that this document records observations rather than standing guidance, names WHAT state or subject was observed, and gives WHEN it was observed as an absolute date, timestamp, or bounded date range -- enough to identify which state the claims describe. A dated audit, measurement, incident record, or survey can qualify. A date by itself or a narrative about past events does NOT qualify and remains subject to every generic criterion. When the marker qualifies, skip ONLY PD-2 maturation, PD-8 duplication, PD-H1 link resolution, N_broken_link_identified_target, and R_stale_anchor; repointing historical evidence can falsify the record. Continue with every other criterion, including PD-3 / PD-7 size signals, PD-4 discoverability, PD-11 ancestor conventions, O_non_ascii_lookalike, and P_foreign_absolute_path. The record role alone does not make an orphan acceptable or SILENT. If the title/header claims the record role or calls the document historical evidence but the opening omits WHAT or WHEN, emit a FAIL (group Hygiene, criterion dated_record_role, taxonomy T_dated_record_missing_provenance, bucket IMPROVE) with remediation to add both pieces; provenance needs author knowledge and is not safe to infer. A bare, undated assertion of historicalness is this FAIL case.
3. PLACEMENT (PD-2 maturation): apply these exclusions BEFORE testing maturity or choosing B, C, or D. A match returns PD-2 PASS without choosing a mature home. (a) STATIC REFERENCE TEXT reads the same whenever it is loaded, such as a platform fact table, format specification, or lookup. It stays a project doc and never becomes a skill (provenance: cohesion-principles.md line 609). (b) MAINTAINER-ONLY MATERIAL must not cross a PUBLISHED-PLUGIN BOUNDARY. Everything under a published plugin directory is copied into a consumer's plugin cache. Ask "who reads this on a machine that is not ours?" If nobody does, keep it as a project doc and make taxonomy D unavailable. (c) DATED RECORD ROLE: a document that qualifies for PD-12's record role returns PD-2 PASS before choosing B, C, or D. For each remaining candidate, route stabilized content by its NATURAL TRIGGER SHAPE (cohesion-principles placement_follows_trigger_shape). A document that opens by naming the ACTIVITY it precedes is TASK-shaped, regardless of the directory that contains its subject matter. Otherwise ask "what is the trigger for needing this?" A TASK-shaped trigger is a VERB the session performs wherever the activity happens. Examples are authoring an X and running an evaluator. Graduate it to a skill (taxonomy B). A LOCATION-shaped trigger is WORKING UNDER A DIRECTORY, such as a config subtree, source directory, or package. Fold it into or reference it from that directory's CLAUDE.md (taxonomy C). Taxonomy C remains the PREFERRED home for genuinely directory-scoped knowledge because it loads only when a file below that directory is touched. If an EXISTING skill owns the non-excluded task-shaped topic, move the content into its references/ (taxonomy D). Knowledge with BOTH shapes prefers the location home (C), and a skill can point at it. Recommend B ONLY for task-shaped knowledge. Do NOT pitch a skill for location-scoped knowledge. If the content is genuinely emerging or unstructured, NONE fires and PD-2 PASSes. All maturation findings are JUDGMENT/IMPROVE -- never FAIL a doc for not yet being a skill.
...
7. ADP back-reference (PD-6): for each CLAUDE.md mention, ask "must the reader read that section to act on this passage?" If yes, the passage is incomplete and FAILS (group ADP, taxonomy G_claude_md_back_reference). If the passage states the rule and its reason inline, the section name is provenance and PASSes. This remains true when phrasing sounds dependent, including "Per the root CLAUDE.md submit gate..." or "See the anti-pattern in CLAUDE.md." PASS shapes: a passage that states a working rule and its reason in full and then names the CLAUDE.md section it came from (the name is attribution); and a passage that opens "Per the root CLAUDE.md <named> gate ..." and then sets out that gate's criteria inline. Only a passage that is incomplete without the upstream section fails.
...
9d. ${standardsClause}
9e. ${disabledClause}
10. DISPOSITION CLASSIFIER. Assign EVERY non-PASS finding a taxonomy id (A-T plus N_user_standard_violation; S_ancestor_convention_violation is the PD-11 ancestor-convention finding from step 9c, group Hygiene; T_dated_record_missing_provenance is the PD-12 incomplete-record-marker finding from step 2a, group Hygiene; N_user_standard_violation is the user-authored-standards finding from step 9d, group Hygiene, disposition driven by the criterion's declared severity -- fail -> SERIOUS, info -> IMPROVE, judgment -> JUDGMENT) and one of four dispositions -- FIX / SERIOUS / IMPROVE / SILENT (K -> SPECIAL). The taxonomy default bucket is a starting point only; decide instance-level against these predicates.
   ...
   IMPROVE (count + one-liners; opt-in): the PD-1 decline notice (A) -- always emitted alongside a NOT-AUDITED verdict, never suppressed, its one-liner naming the auditor that should read the file; a structural move -- graduate to a skill (B), fold into a CLAUDE.md (C), move into an existing skill (D), a CRP split (E), flatten a chained reference (F), remove a back-reference (G), orphan-linking (H), collapse a skill-duplicating doc to a pointer (I) -- I carries a loss-free precondition (fold the doc's unique deltas into the skill BEFORE removal) that no auto-apply pass can satisfy, so it is opt-in, not FIX; README stranded-fact re-home (L), add a generation record (M), add dated-record observation provenance (T); or a trim of TRUE content passing the one-line test.
   SILENT (do NOT surface; no hedging): a do-nothing conclusion ("accept as-is -> PASS"); a validator detection artifact; an accepted structural pattern (an agent-definition file with zero inbound citations, a companion-source PDF, an intentionally human-only orphan). A PD-12 record role is not silent merely because it is a record.
