...
function lanePrompt(f) {
  ...
  // count-shaped claims, which is why it effectively never fired. This is that
  // missing operative instruction. It runs on BOTH dimensions (scope extended
  // 2026-08-07: exact-enumeration counts on code-directory files too).
  const countClaimsClause = `COUNT-SHAPED CLAIMS. Scan the file for every claim that carries a NUMBER about the repo -- "the eleven native systems", "the six unittest suites", "13 tests", "a 7200-line god object", "~40 anchors". Type each one, then act:
   - EXACT-ENUMERATION (the number is what a reader relies on to know the list is COMPLETE -- a definite article plus a plural noun naming a closed set, or a count the reader would use to decide nothing is missing): you MUST verify it. ENUMERATE the actual thing -- grep the registration/definition call and COUNT the call sites, list the test definitions, list the directory entries -- and compare. Wrong number -> emit taxonomy P_stale_factual_claim, disposition FIX: correct the number, information-preservingly (fix the count AND add any missing entry; never just delete the count). On a \`classic\` file severity FAIL as before; on a \`code-directory\` file severity JUDGMENT (only CD-2 gates that dimension).
   - ILLUSTRATIVE-MAGNITUDE (the number conveys SCALE and the claim survives it drifting -- "7200-line god object", "~40 anchors", "resets 10 fields"): do NOT verify and do NOT emit anything on the number. Flag only if the KIND inverts (god-object now decomposed) -- that is CD-4 / I_claim_drift, not this.
   - AMBIGUOUS: treat as illustrative and stay silent. A false count finding costs more than a missed one.
   NEVER verify a count against adjacent prose. Do not accept a comment, docstring, or heading near the code that restates the same number as confirmation: a stale figure is usually echoed in the source's own comment, so **a fact-check performed against adjacent prose CONFIRMS a wrong doc**. Count the executable content -- registrations, definitions, entries. Nearby human-readable text is another copy of the claim, not evidence for it.
   The enumeration must be CLOSED and mechanically identifiable (enum body, registration list, directory listing, test roster). If membership is decided by template / if constexpr / attribute / reflection dispatch, by code generation, or by a governing flag with no live consumer, the count is UNVERIFIABLE -- suppress it, do not estimate.
   UNITS: if the counted noun admits more than one defensible unit (files vs headers vs types; bullets vs distinct tools) and ANY unit makes the count correct, suppress. Fire only when the count is wrong under every reasonable reading.
   Enumerate TRANSITIVELY: follow the registration helpers a call site invokes and count the whole reachable set, then state the hops in the finding. A count stopped one hop short writes a NEW wrong number into the doc under an applied FIX. If the set cannot be enumerated with confidence (reflection, attribute scanning, code generation, runtime assembly), do NOT emit FIX -- downgrade to IMPROVE, give the number you could derive, and name what you could not reach.`

  // INVARIANT vs CODE (CD-2b). Extends H2_inverted_absence from a requires-absent
  // ANCHOR to a stated INVARIANT. Runs on both dimensions because invariants are
  // typically declared in a classic root CLAUDE.md while the violation is in code.
  const invariantClause = `STATED INVARIANT vs CODE (CD-2b, taxonomy H2_inverted_absence -- do NOT invent a new id). The corpus sometimes states an INVARIANT the code must honor ("No silent fallbacks", "Comparability is never assumed"). Check the code against it. This is the one case where the DOC IS RIGHT and the CODE IS WRONG.
   TWO GATES, BOTH REQUIRED -- this is what keeps the check from generating speculation:
     (1) You must be able to quote the invariant VERBATIM from a CLAUDE.md in scope (this file, or one of the ancestor files supplied in step 3.5). Exactly the H-11 rule-extraction posture: no inferred invariants, no generic best practice, no "spirit of" a rule. If you cannot quote it verbatim, do NOT raise the finding.
     (2) The violation must be demonstrable at a CITED code location you actually read (file + symbol). A suspicion with no citation does not fire.
   Emit as group "CodeDir", taxonomy H2_inverted_absence, severity JUDGMENT, disposition SERIOUS. This clause runs on BOTH dimensions -- it is the one CodeDir-group finding that may fire on a \`classic\` file.
   NEVER FIX it. The correction is in code this audit must not touch, and editing the doc to describe the violation would fossilize a bug as documented behavior. REMEDIATION OWNER IS THE CODE AUTHOR: the \`remediation\` names (a) the verbatim invariant, (b) the source CLAUDE.md path, (c) the violating code location, and states that the CODE must be brought back into compliance (or the invariant deliberately retired by a human). It never proposes an edit to the CLAUDE.md.
...
8. DISPOSITION CLASSIFIER. Assign EVERY non-PASS finding a taxonomy id and one of four dispositions -- FIX / SERIOUS / IMPROVE / SILENT (K unclassified -> SPECIAL). The taxonomy default bucket is a starting point only; decide the disposition instance-level against these predicates.
   ...
   Classic-dimension findings use taxonomy A-G/P/Q/K; the count-claim finding (step 4.5) uses P_stale_factual_claim on BOTH dimensions -- exact-enumeration counts only on a code-directory file, always disposition FIX (recount and correct, information-preservingly), never a finding on an illustrative magnitude; the stated-invariant-violated-by-code finding (step 6.5) reuses H2_inverted_absence at severity JUDGMENT, disposition SERIOUS, remediation owned by the code author and never a doc edit; the H-11 ancestor-convention finding (step 3.5) uses R_ancestor_convention_violation (group Hygiene); the user-authored-standards finding (step 3.6) uses N_user_standard_violation (group Hygiene, disposition driven by the criterion's declared severity -- fail -> SERIOUS, info -> IMPROVE, judgment -> JUDGMENT); CodeDir-group findings use H_stale_anchor / H2_inverted_absence / I_claim_drift / I2_line_drift / J_low_value_insight (or K); Density-group findings use L_verbose_in_place / M_extract_to_reference / N_intra_file_redundancy / O_low_value_verbose.
