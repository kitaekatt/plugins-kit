...
//     (kind is the caller's artifact classification. "skill" -> apply the criteria;
//     any OTHER explicit value -> decline with NOT-AUDITED; ABSENT -> the lane
//     self-applies the SKILL.md basename shape test and declines on a non-match.
//     See the decline contract in references/lanes/audit-lane.md step 2a.)
...
const FILE_FINDINGS_SCHEMA = {
  ...
  properties: {
      ...
      items: {
        ...
        properties: {
          ...
          taxonomy: {
            ...
            description: 'canonical suffixed taxonomy id (see the taxonomy table in references/standards/skill-standards.md); "none" for PASS/INFO/JUDGMENT that need no remediation, and for the artifact-shape decline finding',
          },
    ...
    verdict: { type: 'string', enum: ['COMPLIANT', 'NON-COMPLIANT', 'NOT-AUDITED'], description: 'NOT-AUDITED = the criteria were never applied because the file is not a SKILL.md (the artifact-shape decline). It is NOT a passing verdict and must never be reported as one.' },
  },
...
log(review
  ? `Reviewed ${results.length}/${input.files.length} SKILL.md files — ${totals.diffClean} DIFF-CLEAN, ${totals.nonCompliant} NON-COMPLIANT${declined}, ${totals.fail} attributable FAIL; dispositions SERIOUS=${totals.serious} FIX=${totals.fix} IMPROVE=${totals.improve} (${totals.suppressed} pre-existing finding(s) suppressed as not caused by this change; SILENT=${totals.silent} omitted)`
  : `Audited ${results.length}/${input.files.length} SKILL.md files — ${totals.nonCompliant} NON-COMPLIANT${declined}, ${totals.fail} FAIL findings; dispositions SERIOUS=${totals.serious} FIX=${totals.fix} IMPROVE=${totals.improve} (SILENT=${totals.silent} omitted)`)
