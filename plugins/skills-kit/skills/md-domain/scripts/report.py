#!/usr/bin/env python3
"""report.py -- dispatch entry point for the md-domain skill-audit reports (/md-domain audit skill).

Two reports are available; the user picks one by positional argument:

  /md-domain audit skill roster      Markdown roster grouped by location -> type. Per-type
                               implied frontmatter declared once so per-skill rows
                               don't repeat it. Default: <project-root>/tmp/skill-roster.md.

  /md-domain audit skill hierarchy   Interactive HTML hierarchy. Collapsible <details>
...
A trailing path or `-` selects the output destination:
  ...
  /md-domain audit skill roster                       -> default tmp/ path
  /md-domain audit skill roster path/to/file.md       -> that path
  /md-domain audit skill roster -                     -> stdout
...
USAGE = """\
report.py -- corpus-wide report dispatcher for the md-domain skill-audit lane
(reached via /md-domain audit skill). Pick a subcommand:
...
This script is normally invoked by md-domain's skill-audit lane (via
/md-domain audit skill). For single-skill contract audits, see the
audit_skill_md technique in md-domain's SKILL.md (different code path:
scripts/discover_skill.py + the skills_kit_lib.audit validator).
"""
...
def main() -> int:
    ...
    if args.report_type == "hierarchy":
        # Sibling module under skills-kit/skills/md-domain/scripts/.
        sys.path.insert(0, str(Path(__file__).resolve().parent))
