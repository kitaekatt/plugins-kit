...
The `content_allocation:` block below is the load-bearing framework the audits derive from.

## The placement framework

The load-bearing content. The audit skills derive their criteria from this block; `claude-md-standards.md` (in
`md-domain/references/standards/`) is the self-contained, audit-path summary derived from it.
...
content_allocation:
  ...
  principles_applied_to_placement:
    ...
    - id: crp_reader_set
      ...
      note_on_size_threshold: |
        ...
        See `skill-domain/framework.md` "CRP is the test for L2 -> L3 splits" for the operational rule.
  ...
  per_artifact_role:
    ...
    - id: machine_emitted_artifact
      ...
      adp_role: out-of-band leaf; nothing may depend on a machine-emitted artifact as a load-bearing knowledge surface (regeneration may rewrite it wholesale).
      identification: |
      ...
      audit_rules:
        - id: provenance_named
          rule: the generator or session provenance is named -- via the sidecar or the in-file marker. This is the ONLY audit rule for a machine-emitted artifact; the authored-doc criteria (maturation, CRP split, orphan, duplication) are skipped.
  ...
  summarize_and_reference:
    ...
    audit_link: |
      The md-domain audit lanes enforce this convention from the audit side: cross-file / skill-content
...
- **Vocabulary** -- `skill-domain/glossary.md` (in md-domain): CRP, CCP, ADP, SSOT, progressive disclosure, conditional details.
- **CRP for SKILL.md size splits** -- `skill-domain/framework.md` (in md-domain) "CRP is the test for L2 -> L3 splits" section. The placement algorithm here generalizes the CRP test to all placement decisions.
- **Content shape (the orthogonal question)** -- the `authoring-patterns/content-authoring.md` reference (in md-domain): how a fact should be shaped once you know where it lives.
- **The audits that enforce these rules** -- the md-domain audit lanes (skill / claude-md / project-doc / references, via `/md-domain audit skill|claude-md|project-doc|references`). The `project_reference_md` per-artifact role + `skill_maturation_pipeline` here are operationalized by the project-doc audit lane.
