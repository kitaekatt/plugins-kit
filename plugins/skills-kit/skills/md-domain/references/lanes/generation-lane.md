...
- **GENERATE** takes COVERAGE produced by an `analyze` run and writes it up, so
  ...
  It crosses `claude-md`, the artifact the analyze lane feeds. Nothing analyzes
  a codebase and emits skill or project-doc candidates, so there is no coverage
  to generate those from.
...
**The exception is TREE-SCALE generation.** `claude-md` at tree scale binds
`workflow/claude-md-generate.js` because at tree scale the runs are
