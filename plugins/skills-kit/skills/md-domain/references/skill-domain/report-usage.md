# audit_skill lane usage: report subcommands

Full usage reference for the `roster` and `hierarchy` subcommands of the `audit_skill` lane (reached via `/md-domain audit skill`), and their underlying script `scripts/report.py`. Loaded when the agent needs the precise argument set, location semantics, output-shape contract, or implied-frontmatter rules.

For the single-file audit operation (the namesake `/md-domain audit skill <path>` / `list` / `<numbers>` flows), invoke `/md-domain audit skill`; the placement framework lives at `cohesion-principles.md` (in `md-domain/references/`).

## Invocation

Reached through the `/md-domain audit skill` front door (the member's standalone slash command was collapsed). The first argument after `/md-domain audit skill` selects which operation to run; the two report subcommands are:

```
/md-domain audit skill roster                     # markdown to <project-root>/tmp/skill-roster.md (default)
/md-domain audit skill roster tmp/skills.md       # markdown to that path
/md-domain audit skill roster -                   # markdown body printed to stdout (rendered in chat)
/md-domain audit skill hierarchy                  # interactive HTML to <project-root>/tmp/skill-hierarchy.html
/md-domain audit skill hierarchy tmp/x.html       # HTML to that path
...
`/md-domain audit skill hierarchy` delegates to the `render_html(corpus)` function in the sibling `skill_hierarchy_report.py` module. Output is a single self-contained HTML file (no external assets, no JavaScript):
...
- The first ```` ```yaml ```` fenced block in the body, which carries the type contract per the skill-domain framework (`skill-domain/framework.md`).
...
- **example-skill-one** [author: christina] -- Use when doing X ...
- **example-skill-two** [author: christina] -- Use when doing Y ...
```
...
- `(unknown)` skill-type usually means the SKILL.md is missing both a frontmatter `skill-type:` line and a recognized contract root in its body YAML. Treat it as a hint to inspect the file with `/md-domain audit skill <path>`.
