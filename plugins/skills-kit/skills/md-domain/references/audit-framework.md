...
How skills-kit audits things. This is the canonical glossary for audit-related lanes; the `md-domain` audit lanes that consume it -- `audit_skill` (skill-shape audits over the User + Project + Plugins corpus; via `/md-domain audit skill`), `audit_claude_md` (CLAUDE.md cohesion/hygiene; via `/md-domain audit claude-md`), `audit_project_doc` (standalone project-document cohesion/placement; via `/md-domain audit project-doc`), and `audit_references` (cross-reference and orphan audits over markdown corpora; via `/md-domain audit references`) -- each reference it. When a term defined here appears in a lane, the definition lives here; the lane describes only how the audit applies the term.
...
The **legacy AUTO / DISCUSS / SPECIAL** lanes (mechanical / judgment-required / escape-hatch) are still in use by the `audit_references` lane and remain the structural `remediations` lane names in every audit-skill's YAML contract (auto = FIX, discuss = SERIOUS + IMPROVE + SILENT-default, special = K). AUTO maps to FIX; DISCUSS splits into SERIOUS + IMPROVE under the four-disposition model.
...
## How the lanes use this framework

### `audit_references` lane (via `/md-domain audit references`)

Operationalizes the **references-audit** audit-kind. The lane:
...
In framework terms: audit_references's subject is one of `directory | skill | plugin | project`; the primitive it parses is `md`; the rules per composition come from the audit-kind's bindings table in `audit-framework.yaml`. The lane's body documents the operational steps; the rule set is canonical in the YAML.
...
In framework terms: the subject is `skill_md` inside a `skill` composition; the primitives consumed are `skill_md` and `yaml` (the embedded contract block); rules come from the audit-kind's bindings table. The lane's standards doc (`skill-standards.md`) names the same rules that the framework's bindings table references -- the YAML and the standards doc must stay in sync.

### `audit_claude_md` lane (via `/md-domain audit claude-md`)

Operationalizes the **claude-md-audit** audit-kind. The lane:
...
### `audit_project_doc` lane (via `/md-domain audit project-doc`)

Operationalizes the **project-doc-audit** audit-kind. The lane:
...
Decisions that change the framework itself (a new principle, a new severity level, a new bucket) are framework-level changes and need a Dec-N provenance entry in `${CLAUDE_PLUGIN_ROOT}/skills/md-domain/CLAUDE.md` (the merged decision-provenance surface; the pre-restructure per-skill logs are preserved at `${CLAUDE_PLUGIN_ROOT}/skills/md-domain/references/provenance/`), mirroring the audit-driven refinement discipline that shaped skill-authoring.
