...
Decisions that change the framework itself (a new principle, a new severity level, a new bucket) are framework-level changes and need a Dec-N provenance entry in `plugins/skills-kit/skills/md-domain/CLAUDE.md` (the merged decision-provenance surface; the pre-restructure per-skill logs are preserved at `plugins/skills-kit/skills/md-domain/references/provenance/`), mirroring the audit-driven refinement discipline that shaped skill-authoring.
