...
**That is why `generate` exists for `claude-md` alone.** `analyze <directory>`
reads code and emits its input. Nothing analyzes a codebase and emits skill or
project-doc candidates, so those two artifacts are authored. If an analysis is
ever built for them, they gain a generate lane; until then, do not improvise one.
...
WHAT I CAN DO
  ...
  analyze    read one directory and report what a document there should carry --
             its own code for a CLAUDE.md, not its subdirectories' code, each of
             which is its own run; never edits
  ...
  index:
    references:
      ...
      - id: capability_boundaries
        ...
        keywords: [what md-domain does not do, code review, code audit, does not exist, division of labour, external capability, informs review, accountable for covering, subject is a change, subject is a codebase, who verifies a defect]
        summary: The capability boundaries -- md-domain owns a document, code review owns a CHANGE (git-kit, p4-kit), and code audit owns a CODEBASE at rest and DOES NOT EXIST YET. Carries the specification for that absent capability so its absence is not read as an md-domain gap, the one-directional relationship to review (md-domain informs, review raises), and the accountable-for-covering test. Read before letting any lane report a code defect; for the CLAUDE-potential-defects.md file itself, see deferred_defects.
      - id: deferred_defects
        path: references/deferred-defects.md
        keywords: [CLAUDE-potential-defects.md, deferred defect, known defect, gotcha not fixed now, known gotcha, defect deliberately deferred, sidecar file, defect placement rule, source field, verified field, why_deferred, schema v2, hazard-durability route]
        summary: The primary definition, placement rule, and YAML schema for CLAUDE-potential-defects.md -- a sidecar that records known defects and gotchas severe enough to fix but not being addressed now. The coverage lane's hazard-durability rejection is one admission route among several (triage, review, ordinary work); every route records source and verified per entry. Read before creating or writing to any CLAUDE-potential-defects.md.
      - id: coverage_gap
      ...
      - id: generation_lane
        ...
        summary: The ONE producing procedure, shared by author and generate and parameterized by artifact -- confirm the artifact, resolve placement via cohesion-principles, apply the artifact's standards doc in the PRODUCING direction, shape per the authoring-patterns cluster, validate. Also the coverage intake that distinguishes generate from author, tree generation, and the retention rules regeneration runs under.
      - id: coverage_lane
