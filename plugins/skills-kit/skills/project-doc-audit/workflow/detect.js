...
//              cited_by?: string[] } ],
//     (kind/lines/approx_tokens/inbound_citations/cited_by come from discover.py
...
//     degrades gracefully when a discover.py signal is absent -- it counts the
...
function lanePrompt(f) {
  ...
  const orphanClause = f.inbound_citations === 0
    ...
    : typeof f.inbound_citations === 'number'
      ...
      : `No inbound-citation signal was provided (e.g. a review-mode subject-lens call, where the discover.py citer scan does not run). Orphan status is UNKNOWABLE without that scan, so PD-4 is N/A here -- do NOT emit an H_orphan finding.`

  const hasSize = typeof f.lines === 'number' || typeof f.approx_tokens === 'number'
  const sizeClause = !hasSize
    ? `Size signals were not precomputed (no discover.py record). Count the body yourself in step 1 and run the PD-3 unitary-reading-task evaluation ONLY if the body is clearly over ~500 lines / ~3000 tokens AND its sections fire on genuinely different sub-triggers -> JUDGMENT E_crp_split (group CRP). Size alone is NEVER a FAIL.`
